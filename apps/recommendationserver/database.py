...
#     cursor.execute("CREATE TABLE `chrisrecommend`.`user_recommendations` (`userID` INT NOT NULL,`tmdbID` INT NOT NULL,`modelID` INT NOT NULL,`timestamp` DATETIME NULL);")
#     cursor.execute("ALTER TABLE `chrisrecommend`.`movies` CHANGE COLUMN `movieID` `movieID` INT NOT NULL AUTO_INCREMENT ;")
#     cursor.execute("ALTER TABLE `chrisrecommend`.`users` CHANGE COLUMN `userID` `userID` INT NOT NULL AUTO_INCREMENT ;")
