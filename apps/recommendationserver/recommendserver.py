...
def getmylist():
    ...
    if (userID!='0'):
        ...
        movieists = utils.fetchmylist(movieIDs)
...
def getcontentre():
    ...
    if (userID!='0'):
        ...
        cursor.execute(getuserIdquery)
        movieIDs=cursor.fetchall()
        movieists = utils.contentrecommendations(movieIDs,userID)
        random.shuffle(movieists)
        allrecommendations.append(movieists)
        movieists = utils.implicitcollaborativerecommendations(userID)
        random.shuffle(movieists)
        allrecommendations.append(movieists)
        movieists = utils.explicitcollaborativerecommendations(userID)
        random.shuffle(movieists)
        allrecommendations.append(movieists)
        movieists = utils.implicitneuralcollaborativerecommendations(userID)
        random.shuffle(movieists)
        allrecommendations.append(movieists)
        movieists = utils.explicitneuralcollaborativerecommendations(userID)
        random.shuffle(movieists)
        allrecommendations.append(movieists)
        getuserIdquery = 'select tmdbID from user_movielist WHERE userID = %s;' % (userID)
        cursor.execute(getuserIdquery)
...
def gettrending():
        gettmdbquery = 'SELECT tmdbid from (SELECT * FROM daily_update limit 8) as f'
...
def getmovies():
        gettmdbquery = 'SELECT tmdbid from (SELECT * FROM daily_update limit 100) as v;'
        cursor.execute(gettmdbquery)
        movieIDs = cursor.fetchall()
        movieists = utils.fetchmylist(movieIDs)
